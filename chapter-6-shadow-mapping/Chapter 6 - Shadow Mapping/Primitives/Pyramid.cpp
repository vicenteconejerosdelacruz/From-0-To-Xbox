...
void Pyramid::Initialize(UINT numFrames, ComPtr<ID3D12Device2>	d3dDevice, ComPtr<ID3D12GraphicsCommandList2> commandList, ComPtr<ID3D12Resource>	directionalLightShadowMap, ComPtr<ID3D12Resource>	spotLightShadowMap, ComPtr<ID3D12Resource> pointLightShadowMap) {
	...
	//create the constant buffer view descriptors for each frame, add one descriptor slot for the textures and three more for ths shadowmaps
	//crea los descriptors de constant buffer view por cada frame, agrega un slot de descriptores para las texturas y tres mas para los shadowmaps
	CD3DX12_CPU_DESCRIPTOR_HANDLE cbvCpuHandle = InitializeConstantsBufferView(numFrames, 4U, d3dDevice, cbvData);
	...
	CD3DX12_CPU_DESCRIPTOR_HANDLE directionalLightShadowMapCpuHandle(pyramidCpuHandle);
	...
	auto createVSTask = DX::ReadDataAsync(L"Shaders\\VertexShaders\\Lighting.cso").then([this](std::vector<byte>& fileData) {
		vertexShader = fileData;
	});
	auto createPSTask = DX::ReadDataAsync(L"Shaders\\PixelShaders\\Lighting.cso").then([this](std::vector<byte>& fileData) {
	...
	auto createPipelineStateTask = (createPSTask && createVSTask).then([this, d3dDevice]() {

		{
			...
			rangeSRV.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 4, 0);
...
void Pyramid::UpdateConstantsBuffer(UINT backBufferIndex, BOOL useBlinnPhong, XMMATRIX viewProjection, XMVECTOR eyePos, XMVECTOR ambientLightColor, XMVECTOR directionalLightDirection, XMVECTOR directionalLightColor, XMVECTOR spotLightPosition, XMVECTOR spotLightColor, XMVECTOR spotLightDirectionAndAngle, XMVECTOR spotLightAttenuation, XMVECTOR pointLightColor, XMVECTOR pointLightPosition, XMVECTOR pointLightAttenuation, BOOL shadowMapsEnabled, XMMATRIX directionalLightShadowMapProjection, XMFLOAT2 directionalLightShadowMapTexelInvSize, XMMATRIX spotLightShadowMapProjection, XMFLOAT2 spotLightShadowMapTexelInvSize, XMMATRIX pointLightShadowMapProjection[6])
{
	...
	constants.shadowMapsTextureIndex = 1;
