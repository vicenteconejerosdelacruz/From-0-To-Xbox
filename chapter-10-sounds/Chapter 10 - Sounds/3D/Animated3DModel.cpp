...
void Animated3DModel::Initialize(UINT numFrames, ComPtr<ID3D12Device2>	d3dDevice, ComPtr<ID3D12GraphicsCommandList2> commandList, ComPtr<ID3D12Resource>	directionalLightShadowMap, ComPtr<ID3D12Resource>	spotLightShadowMap, ComPtr<ID3D12Resource> pointLightShadowMap, std::string path) {
	...
	for (UINT meshIndex = 0; meshIndex < aiModel->mNumMeshes; meshIndex++) {
		...
		if (normalMapName.length > 0) {
			...
			CreateTextureResource(d3dDevice, commandList, (const LPWSTR)normalMapPath.c_str(), normalMapTexture, normalMapTextureUpload, normalMapSrvDesc);
		...
		ConstantsBufferViewData<SkinningShaderConstants> cbvMeshData;
	...
	auto createVSTask = DX::ReadDataAsync(L"Shaders\\VertexShaders\\Skinning.cso").then([this](std::vector<byte>& fileData) {
		vertexShader = fileData;
	});
	auto createPSTask = DX::ReadDataAsync(L"Shaders\\PixelShaders\\Skinning.cso").then([this](std::vector<byte>& fileData) {
	...
	auto createPipelineStateTask = (createPSTask && createVSTask).then([this, d3dDevice]() {
		...
		static const D3D12_INPUT_ELEMENT_DESC inputLayout[] =
		{
			...
			{ "BLENDINDICES", 0, DXGI_FORMAT_R32G32B32A32_UINT, 0, offsetof(VertexType,BoneIds), D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 },
			{ "BLENDWEIGHT", 0, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, offsetof(VertexType,BoneWeights), D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 },
		};
	...
	//load the vertex/pixel shaders in async way
	//cargar el vertex/pixel shader de manera asincrona
	auto createShadowMapVSTask = DX::ReadDataAsync(L"Shaders\\VertexShaders\\SkinningShadowMap.cso").then([this](std::vector<byte>& fileData) {
		shadowMapVertexShader = fileData;
	});
	auto createShadowMapPSTask = DX::ReadDataAsync(L"Shaders\\PixelShaders\\SkinningShadowMap.cso").then([this](std::vector<byte>& fileData) {
		shadowMapPixelShader = fileData;
	});
	auto createShadowMapAlphaCutVSTask = DX::ReadDataAsync(L"Shaders\\VertexShaders\\SkinningShadowMapAlphaCut.cso").then([this](std::vector<byte>& fileData) {
		shadowMapAlphaCutVertexShader = fileData;
	});
	auto createShadowMapAlphaCutPSTask = DX::ReadDataAsync(L"Shaders\\PixelShaders\\SkinningShadowMapAlphaCut.cso").then([this](std::vector<byte>& fileData) {
...
void Animated3DModel::UpdateConstantsBuffer(UINT backBufferIndex, XMMATRIX viewProjection, XMVECTOR eyePos, XMVECTOR ambientLightColor, XMVECTOR directionalLightDirection, XMVECTOR directionalLightColor, XMVECTOR spotLightPosition, XMVECTOR spotLightColor, XMVECTOR spotLightDirectionAndAngle, XMVECTOR spotLightAttenuation, XMVECTOR pointLightColor, XMVECTOR pointLightPosition, XMVECTOR pointLightAttenuation, BOOL shadowMapsEnabled, XMMATRIX directionalLightShadowMapProjection, XMFLOAT2 directionalLightShadowMapTexelInvSize, XMMATRIX spotLightShadowMapProjection, XMFLOAT2 spotLightShadowMapTexelInvSize, XMMATRIX pointLightShadowMapProjection[6], BOOL normalMappingEnabled) {
	...
	SkinningShaderConstants constants;
